...
class CustomProxyMiddleware:
    ...
    def get_random_proxy(cls):
        available = [p for p in settings.proxies if p not in cls().failed_proxies]
    ...
    def process_exception(self, request, exception, spider):
        ...
        if current_proxy in settings.proxies:
            self.failed_proxies.add(current_proxy)

        proxy = self.get_random_proxy()
        if proxy:
            return request.replace(
                meta={"proxy": proxy, "download_timeout": 30}, dont_filter=True
            )
        else:
            return None